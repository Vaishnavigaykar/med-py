...
def get_predicted_value(user_symptoms):
    # Convert user symptoms to the format expected by the model
    symptom_vector = np.zeros(len(sym_des))  # Assuming sym_des is a DataFrame of symptoms
    for symptom in user_symptoms:
if symptom in sym_des[['Symptom_1', 'Symptom_2', 'Symptom_3', 'Symptom_4']].values:
            symptom_vector[sym_des[sym_des['Symptom'] == symptom].index[0]] = 1

    # Make prediction
    prediction = svc.predict([symptom_vector])
    predicted_disease = prediction[0]  # Assuming the model returns a single prediction
